...
api: true
explorer: false
permalink: 'cluster_endpoint_overview.html'
...
# Modify  field:
curl -X PATCH "https://<mgmt-ip>/api/cluster" -d '{ "active_directory": {"username": "administrator", "password": "password", "name": "adaccount", "fqdn": "test.com", "force_account_overwrite" : true, "security": {"advertised_kdc_encryptions": ["des"]}}}'
