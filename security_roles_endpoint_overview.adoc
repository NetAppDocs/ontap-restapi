...
api: true
explorer: false
permalink: 'security_roles_endpoint_overview.html'
...
Additionally, Artificial Intelligence Data Engine (AIDE) defines the following predefined cluster-scoped roles: "data-engineer", "data-scientist", and "aide-rag".

All of the above predefined roles can be retrieved by calling a GET request on the _/api/security/roles_ API and can be assigned to user accounts. See the examples for _api/security/accounts_.
...
=== Creating a custom role with an AIDE resource-qualified endpoint
