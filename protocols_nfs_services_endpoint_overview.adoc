...
api: true
explorer: false
permalink: 'protocols_nfs_services_endpoint_overview.html'
...
# The call:
curl -X GET curl -X GET "https://<mgmt-ip>/api/protocols/nfs/services/{svm.uuid}?protocol_access_rules.volume=testvol12&protocol_access_rules.client_ip=1.2.3.4&protocol_access_rules.auth_type=sys&fields=protocol_access_rules&return_timeout=15&return_records=true"
