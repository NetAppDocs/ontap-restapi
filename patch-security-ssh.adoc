...
|host_key_algorithms
|array[string]
a|Host key algorithms. The host key algorithms 'ssh_ed25519' and 'ssh_rsa' can be configured only in non-FIPS mode.
...
|login_grace_time
|integer
a|The SSH connection login grace time allowed for the connection.
...
| 10682426
| Failed to modify the SSH connection login grace time.
